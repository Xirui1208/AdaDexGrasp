...
pointnet2_path = os.path.abspath(os.path.join(script_dir, '..', 'Pointnet2_PyTorch'))
if os.path.exists(pointnet2_path):
    sys.path.append(pointnet2_path)
else:
    alt = "../Pointnet2_PyTorch"
...
def _sample_points_helper(pc, num_points):
    if pc is None or pc.shape[0] == 0:
        return torch.zeros(num_points, pc.shape[1] if pc is not None else 0, device=pc.device if pc is not None else 'cpu', dtype=pc.dtype if pc is not None else torch.float)
    if pc.shape[0] > num_points:
        indices = torch.randperm(pc.shape[0])[:num_points]
        pc = pc[indices]
    elif pc.shape[0] < num_points:
        if pc.shape[0] == 0:
            return torch.zeros(num_points, pc.shape[1], device=pc.device, dtype=pc.dtype)
...
def _augment_data_helper(xyz, normals):
    ...
    scale = (torch.rand(1, device=xyz.device) * 0.2 + 0.9).item()
    xyz = xyz * scale
...
def process_point_cloud(
    ...
    seg_id_hand = 2.0
    seg_id_object = 3.0
    num_contact_categories = 6
    object_mask = (raw_pc[:, 6] == seg_id_object)
    contact_intensities = raw_pc[:, 7:13] if raw_pc.shape[1] >= 13 else torch.zeros(raw_pc.shape[0], num_contact_categories, device=raw_pc.device)
    contact_mask = (contact_intensities.max(dim=1)[0] > 0.1)
    ...
    if object_points.shape[0] > 0:
        max_dist = torch.max(torch.sqrt(torch.sum((object_points - centroid)**2, dim=1)))
    else:
        max_dist = torch.max(torch.sqrt(torch.sum(all_xyz**2, dim=1)))
    if max_dist > 1e-6:
        all_xyz = all_xyz / max_dist
    raw_pc[:, :3] = all_xyz
    if augment:
    ...
    contact_intensities = contact_intensities if contact_intensities.shape[1] == num_contact_categories else torch.zeros(raw_pc.shape[0], num_contact_categories, device=raw_pc.device)
    raw_pc_with_normals = torch.cat([all_xyz, raw_pc[:, 3:6], normals, seg_ids_for_norm.unsqueeze(1), contact_intensities], dim=1)
    seg_ids = raw_pc_with_normals[:, 9]
    contact_intensities_in_pc = raw_pc_with_normals[:, 10:16]
    pc_hand = raw_pc_with_normals[seg_ids == seg_id_hand]
    pc_object = raw_pc_with_normals[seg_ids == seg_id_object]
    contact_mask_split = (contact_intensities_in_pc.max(dim=1)[0] > 0.1)
    ...
    if pc_contact.shape[0] > 0:
        contact_intensities_contact = contact_intensities_in_pc[contact_mask_split]
        is_contact_con_features = contact_intensities_contact
    ...
    final_object_pc = torch.cat([pc_object_sampled[:, :9], pc_object_sampled[:, 10:16]], dim=1)
...
if __name__ == '__main__':
    DATA_PATH = "ECCV/classifier_data/classifier.npz"
    EPOCHS = 200
    BATCH_SIZE = 128
    LEARNING_RATE = 3e-4
    WEIGHT_DECAY = 5e-4
    ...
    best_test_acc = 0.0
    best_epoch = 0
    best_model_path = "ECCV/classifier_data/best_classifier_model.pth" 
    patience = 20
    epochs_no_improve = 0
