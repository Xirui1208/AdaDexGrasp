...
from classifier import GraspSuccessClassifier, process_point_cloud
import os
import pathlib
import hydra
...
spec1 = importlib.util.spec_from_file_location("cvae_model1", str(train_model1_file_path))
spec2 = importlib.util.spec_from_file_location("cvae_model2", str(train_model2_file_path))

dp3_train = importlib.util.module_from_spec(spec)
cvae_model1 = importlib.util.module_from_spec(spec1)
cvae_model2 = importlib.util.module_from_spec(spec2)
sys.modules["dp3_train"] = dp3_train
sys.modules["cvae_model1"] = cvae_model1
sys.modules["cvae_model2"] = cvae_model2
spec.loader.exec_module(dp3_train)
spec1.loader.exec_module(cvae_model1)
spec2.loader.exec_module(cvae_model2)

DP3 = dp3_train.DP3    
CVAE_model1 = cvae_model1.Network
CVAE_model2 = cvae_model2.NetworkRegression
...
class hand_env(BaseTask,TacSLTaskImageAugmentation,TacSLSensors):
    ...
    def _collect_pointclouds(self):
        ...
        contact_label_threshold = 0.1
        object_mask_fps = (points_fps[:, :, 6] == self.segmentation_id['object'])
        seg_buffer = points_fps[:, :, 6].clone()
        contact_intensity_buffer = torch.zeros(
            (self.num_envs, points_fps.shape[1], 6),
            device=self.device,
            dtype=points_fps.dtype,
        )
        ...
        if self.contact_calculation_enabled:
            ...
            if joint_angles.shape[1] == self.collision_model.n_dofs:
                ...
                gaussian_sigma = 0.1 
                link_indices_map = {
                    'robot0:thumb_gelsight': 0,
                    'robot0:ff_gelsight': 1,
                    'robot0:mf_gelsight': 2,
                    'robot0:rf_gelsight': 3,
                    'robot0:lf_gelsight': 4,
                    'robot0:palm_gelsight': 5
                }

                # Match collection: palm is written first, then fingers override it.
                for link_name, contact_dim in reversed(list(link_indices_map.items())):
                    ...
                    contact_env_indices = torch.where(has_contact_force[:, contact_dim])[0]
                    
                    if contact_env_indices.numel() > 0:
                        dists_for_contact_envs = masked_dist[contact_env_indices]
                        _, topk_indices = torch.topk(dists_for_contact_envs, min(30, dists_for_contact_envs.shape[1]), dim=1, largest=False)
                        seg_buffer[contact_env_indices.unsqueeze(1), topk_indices] = 11.0 + contact_dim
                        for env_idx_in_batch, env_global_idx in enumerate(contact_env_indices):
                            top_distances = dists_for_contact_envs[env_idx_in_batch, topk_indices[env_idx_in_batch]]
                            min_dist = top_distances.min()
                            max_dist = top_distances.max()
                            if max_dist > min_dist:
                                normalized_distances = (top_distances - min_dist) / (max_dist - min_dist)
                            else:
                                normalized_distances = torch.zeros_like(top_distances)
                            gaussian_intensity = torch.exp(-(normalized_distances ** 2) / (2 * gaussian_sigma ** 2))
                            contact_intensity_buffer[env_global_idx, topk_indices[env_idx_in_batch], contact_dim] = gaussian_intensity
        ...
        # Column 6 keeps one legacy contact ID (11-16); columns 7:13 keep all intensities.
        points_fps[:, :, 6] = seg_buffer
        points_fps = torch.cat([points_fps, contact_intensity_buffer], dim=-1)
        mask_hand = (points_fps[:,:,6] == self.segmentation_id["hand"])
        mask_object = object_mask_fps
        mask_contact_thumb = points_fps[:, :, 7] > contact_label_threshold
        mask_contact_ff = points_fps[:, :, 8] > contact_label_threshold
        mask_contact_mf = points_fps[:, :, 9] > contact_label_threshold
        mask_contact_rf = points_fps[:, :, 10] > contact_label_threshold
        mask_contact_lf = points_fps[:, :, 11] > contact_label_threshold
        mask_contact_palm = points_fps[:, :, 12] > contact_label_threshold
        ...
        self.debug_points = points_fps.clone()
...
def _prepare_object_points_for_cvae(object_points_world, object_state_world, target_points=1024):
    """Match CVAE training: world-to-object rigid transform in meters, then sample/pad."""
    if object_points_world.ndim != 2 or object_points_world.shape[1] < 3:
        raise ValueError("object_points_world must have shape (N, >=3)")
    if object_points_world.shape[0] == 0:
        raise RuntimeError("Cannot generate a grasp pose because no object points were observed")

    object_xyz_world = object_points_world[:, :3]
    object_pos_world = object_state_world[:3]
    object_quat_xyzw = object_state_world[3:7]
    object_quat_xyzw = object_quat_xyzw / torch.norm(object_quat_xyzw).clamp_min(1e-8)

    centered_xyz = object_xyz_world - object_pos_world.unsqueeze(0)
    expanded_object_quat = object_quat_xyzw.unsqueeze(0).expand(centered_xyz.shape[0], -1)
    object_xyz_local = quat_rotate_inverse(expanded_object_quat, centered_xyz)

    num_points = object_xyz_local.shape[0]
    if num_points > target_points:
        indices = torch.randperm(num_points, device=object_xyz_local.device)[:target_points]
        object_xyz_local = object_xyz_local[indices]
    elif num_points < target_points:
        indices = torch.randint(
            0, num_points, (target_points,), device=object_xyz_local.device
        )
        object_xyz_local = object_xyz_local[indices]

    return object_xyz_local
   
def prepare_classifier_input(full_pc, device=None, num_points_hand=1024, num_points_object=1024, 
                             rgb_mean=None, rgb_std=None, rgb_scale_255=False):

    if full_pc.dim() == 2:
        full_pc = full_pc.unsqueeze(0)
    assert full_pc.dim() == 3 and full_pc.size(-1) >= 7, "full_pc must be (B,N,>=7). Format: [x,y,z,r,g,b,seg_id] or [x,y,z,r,g,b,seg_id,contact_0-5]"
    if device is None:
...
class GraspEvaluator:
    def __init__(self, args):
        ...
        self.contact_map_model = CVAE_model1(point_cloud_dim=3,num_classes=7,)
        ...
        self.contact_pose_model = CVAE_model2(point_cloud_dim=4,robot_state_dim=29)
    ...
    def set_hand_pose_from_cvae(self, env_ids):
        ...
        with torch.no_grad():
            for i in env_ids:
                ...
                seg_ids = pc_env_i[:, 6]
                contact_id_mask = (seg_ids >= 11.0) & (seg_ids <= 16.0)
                object_mask = (seg_ids == obj_seg_id) | contact_id_mask
                object_points_full = pc_env_i[object_mask]
                object_state = self.task.root_state_tensor[self.task.object_indices[env_id]]
                object_points_xyz_local = _prepare_object_points_for_cvae(
                    object_points_full,
                    object_state,
                    target_points=1024,
                )
                model_input_m1 = object_points_xyz_local.unsqueeze(0)
                ...
                model_input_m2 = torch.cat(
                    [object_points_xyz_local, predicted_classes.float().unsqueeze(-1)],
                    dim=1
                ).unsqueeze(0)
                ...
                obj_pos_w = object_state[0:3].unsqueeze(0)
                obj_rot_w = object_state[3:7].unsqueeze(0)
    ...
    def reset(self, env_ids):
        ...
        new_pos_z = table_surface_z + self.task.object_init_z[env_ids] + 0.2
    ...
    def get_force_pc(self):
        ...
        points_fps, others = self.task._collect_pointclouds()
        point_cloud = self.task.debug_points.clone()
    ...
    def run_evaluation(self,max_adjustments=10):
        ...
        final_success_contacts = []
        final_fail_contacts = []
        final_success_finger_counts = []
        final_fail_finger_counts = []
        lift_decision_contact = torch.zeros(num_envs, device=self.device)
        lift_contact_recorded = torch.zeros(num_envs, dtype=torch.bool, device=self.device)
        lift_decision_finger_count = torch.zeros(num_envs, device=self.device)
        ...
        while torch.any(env_states != 4):
            ...
            if len(pending_env_ids) > 0:
                ...
                for i, env_id in enumerate(pending_env_ids):
                    ...
                    if is_success:
                        ...
                        if not lift_contact_recorded[env_idx]:
                            hand_part_ids = [
                                self.task.shadow_body_ids_env['thumb_gelsight'],
                                self.task.shadow_body_ids_env['ff_gelsight'],
                                self.task.shadow_body_ids_env['mf_gelsight'],
                            ...
                            for part_id in hand_part_ids:
                                fvec = self.task.pairwise_contact_force[env_idx, object_body_id, part_id]
                                fmag = torch.norm(fvec, p=2)
                                force_sum += fmag
                                if fmag > contact_threshold:
                                    finger_count += 1
                            lift_decision_contact[env_idx] = force_sum / len(hand_part_ids)
                            lift_decision_finger_count[env_idx] = finger_count
                            lift_contact_recorded[env_idx] = True

                    else:
                        env_states[env_idx] = 3 
            ...
            if torch.any(finished_lift_mask):
                ...
                env_states[finished_lift_ids] = 2
            ...
            if torch.any(finished_hold_mask):
                ...
                for i, env_id in enumerate(finished_hold_ids):
                    ...
                    env_states[env_idx] = 4 
                    if lift_contact_recorded[env_idx]:
                        c = lift_decision_contact[env_idx].item()
                        n = lift_decision_finger_count[env_idx].item()
                        if total_successes_per_env[env_idx] > 0:
                            final_success_contacts.append(c)
                            final_success_finger_counts.append(n)
                        else:
                            final_fail_contacts.append(c)
                            final_fail_finger_counts.append(n)
            ...
            if len(prepare_env_ids) > 0:
                ...
                if len(needs_next_trial_ids) > 0:    
                    ...
                    for env_id_tensor in needs_next_trial_ids:
                        ...
                        pc_input_norm = pc_obs_adj[env_idx, :, :7].unsqueeze(0)
                    ...
                    seg_ids = point_cloud[:, :, 6]
                    contact_id_mask = (seg_ids >= 11.0) & (seg_ids <= 16.0)
                    object_points_mask = (seg_ids == object_seg_id) | contact_id_mask
        ...
        if len(final_success_contacts) > 0:
            avg_success_contact = sum(final_success_contacts) / len(final_success_contacts)
        else:
            avg_success_contact = 0.0

        if len(final_fail_contacts) > 0:
            avg_fail_contact = sum(final_fail_contacts) / len(final_fail_contacts)
        else:
            avg_fail_contact = 0.0
...
if __name__ == '__main__':
    class Args:
        ...
        classifier_model_path = "your_classifier_model_checkpoint.pth"
        diffusion_model_path = "your_diffusion_model_checkpoint.pth"
        contact_map_model_path = "your_contact_map_model_checkpoint.pth"
        contact_pose_model_path = "your_contact_pose_model_checkpoint.pth"
        max_adjustments = 5
        sim_device = "cuda:0"
