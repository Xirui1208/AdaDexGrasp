...
from tasks.tacsl_sensors.tactile_utils.gelsight_render import gelsightRender
...
class TactileBase:
    ...
    def configure_compliant_dynamics(self, actor_handle, elastomer_link_name,
        ...
        if not isinstance(compliance_stiffness, Iterable):
            compliance_stiffness = [compliance_stiffness] * len(self.envs)
        if not isinstance(compliant_damping, Iterable):
            compliant_damping = [compliant_damping] * len(self.envs)
...
class CameraSensor:
    ...
    def setup_env_cameras(self, env_ptr, camera_spec_dict):
        ...
        for name, camera_spec in camera_spec_dict.items():
            ...
            num_actors = self.gym.get_actor_count(env_ptr)  # 获取当前环境中的 actor 数量
            for actor_idx in range(num_actors):
                actor_name = self.gym.get_actor_name(env_ptr, actor_idx)  # 获取 actor 的名称
            # if actor_handle == -1:
            #     raise RuntimeError(f"Actor '{camera_spec['actor_name']}' not found in environment")

            robot_body_handle = self.gym.find_actor_rigid_body_handle(env_ptr, actor_handle, camera_spec["attach_link_name"])
            if robot_body_handle == -1:
                raise RuntimeError(f"Rigid body '{camera_spec['attach_link_name']}' not found for actor '{camera_spec['actor_name']}'")
...
class TactileRGBSensor(TactileBase, CameraSensor):
    ...
    def visualize_tactile_camera_frame(self, tip_to_cam_pos, tip_to_cam_quat):
        ...
        elastomer_tip_link_handle = self.gym.find_actor_rigid_body_handle(self.envs[0], gelsight_handle,
                                                                          'elastomer_tip')
    ...
    def get_camera_image_tensors_dict(self):
        ...
        # print("NOMINAL",self.nominal_tactile)
        if hasattr(self, 'has_tactile_rgb') and self.nominal_tactile:
            for k in self.nominal_tactile:
                ...
                # print(f"Computing tactile RGB for {k} with nominal depth image: {self.nominal_tactile[k]}")
                # print(f"Depth image delta for {k}: {depth_image}")
                # 分别使用cv2展示nominal_tactile和camera_image_tensors_dict[k]
                import cv2
                import numpy as np
                nominal_image = self.nominal_tactile[k].cpu().numpy().squeeze()
                camera_image = camera_image_tensors_dict[k].cpu().numpy().squeeze()
                depth_image_np = depth_image.cpu().numpy().squeeze()
                # cv2.imshow(f'Camera Image {k}', camera_image)
                # cv2.imshow(f'Nominal Tactile Image {k}', nominal_image)
                # cv2.imshow(f'Depth Image Delta {k}', depth_image_np)
                # cv2.waitKey(10)
...
class TactileFieldSensor(TactileBase):
    ...
    def setup_tactile_force_field(self, sdf_tool, num_tactile_rows, num_tactile_cols, tactile_shear_field_configs):
        ...
        if sdf_tool == 'physx':
            ...
            # 验证是否acquire成功
            if _sdf_tensor is None:
                raise RuntimeError("Failed to acquire SDF view tensor. Ensure that the PhysX SDF feature is enabled and properly configured.")
    ...
    def get_elastomer_to_tip_transform(self, actor_name, elastomer_link_name, elastomer_tip_link_name):
        ...
        elastomer_link_handle = self.gym.find_actor_rigid_body_handle(self.envs[0], elastomer_parent_actor_handle, elastomer_link_name)
        elastomer_tip_link_handle = self.gym.find_actor_rigid_body_handle(self.envs[0], elastomer_parent_actor_handle,
    ...
    def query_distance_and_normal_with_physx(self, query_points, visualize=False):
        ...
        """
        # print("1")
        # print("query point",query_points)
    ...
    def post_process_shear_field_configs(self, tactile_shear_field_configs):
        def get_link_handle(actor_name, link_name):
            link_handle = self.gym.find_actor_rigid_body_handle(
                self.envs[0], self.actor_handles[actor_name], link_name)
            return link_handle
    ...
    def get_penalty_based_tactile_forces(self, indenter_link_id, elastomer_link_id):
        ...
        # print("TEST")
        # print("depth",depth)
        # print("Normal",tactile_normal_force)
        # print("Shear",tactile_shear_force)
