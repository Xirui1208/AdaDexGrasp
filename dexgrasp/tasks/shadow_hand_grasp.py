...
class ShadowHandGrasp(BaseTask,TacSLTaskImageAugmentation,TacSLSensors):
    def __init__(self, cfg, sim_params, physics_engine, device_type, device_id, headless,
        ...
        self.contact_reward_scale = self.cfg["env"].get("contactRewardScale", 0.1)
        self.force_balance_penalty_scale = self.cfg["env"].get("forceBalancePenaltyScale", 0.05)
        ...
        num_obs = 236 + 64 - 30 + 18
        if self.use_tactile_image:
            num_obs = 236 + 64 - 30 + 18 + self.num_tactile_sensors * self.tactile_image_height * self.tactile_image_width * self.tactile_image_channels // (self.ssr * self.ssr)
        self.num_obs_dict = {
            "full_state": num_obs
        }
        # self.num_hand_obs = 66 + 65 + 24 + 6 + 18  # 167 =  22*3 + 65 + 24
        self.up_axis = 'z'
        self.fingertips = ["ffdistal", "mfdistal", "rfdistal", "lfdistal",
                           "thdistal"]
...
#------------tacsl module --------------#
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        # asset_root = "../assets/urdf/shadow_hand_tacsl/urdf"
        ...
        all_dofs = torch.arange(self.num_shadow_hand_dofs, device=self.device, dtype=torch.long)
        passive_mask = torch.ones(self.num_shadow_hand_dofs, dtype=torch.bool, device=self.device)
        passive_mask[self.actuated_dof_indices] = False
        self.passive_dof_indices = all_dofs[passive_mask]  # 预计 4
    ...
    def compute_reward(self, actions, id=-1):
        ...
        self.rew_buf[:], self.reset_buf[:], self.reset_goal_buf[:], self.progress_buf[:], self.successes[:], self.current_successes[:], self.consecutive_successes[:] = compute_hand_reward(
            ...
            self.contact_reward_scale, self.force_balance_penalty_scale
        )
    ...
    def compute_full_state(self, asymm_obs=False):
        ...
        self.obs_buf[:,2 * self.num_shadow_hand_dofs:3 * self.num_shadow_hand_dofs] = self.force_torque_obs_scale * self.dof_force_tensor[:, :22]
        ...
        # if(self.use_tactile_image):
        #     tactile_image_start = tactile_obs_start + 18
        #     images = self.get_camera_image_tensors_dict()
        #     tactile_image_list = []
        #     for k in self.tactile_ig_keys:
        #         images[k] = images[k][:, ::self.ssr, ::self.ssr]
        #         images[k] = images[k].reshape(self.num_envs, -1) / 255.0
        #         tactile_image_list.append(images[k])
        #     tactile_images = torch.cat(tactile_image_list, dim=-1)
        #     self.obs_buf[:, tactile_image_start:self.num_obs] = tactile_images
...
def compute_hand_reward(
        ...
        force_balance_penalty_scale: float
):
