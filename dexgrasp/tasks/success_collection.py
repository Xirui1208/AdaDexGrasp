...
class ShadowHandRandomLoadVision(BaseTask,TacSLTaskImageAugmentation,TacSLSensors):
    def __init__(self, cfg, sim_params, physics_engine, device_type, device_id, headless,
        ...
        self.save_data_dir = "DATA/supp/wallclock_772f1"
        os.makedirs(self.save_data_dir, exist_ok=True)
        self.save_data_path = os.path.join(self.save_data_dir, "wallclock_772f1-suc.npz")
...
# 加18個tactile finger force
    ...
    def _collect_pointclouds(self):
        ...
        # # 选择第一个环境的点云
        # object_mask = (points_batch[0, :, 6] == self.segmentation_id['object'])  # 选择第一个环境
        # object_pointcloud = points_batch[0][object_mask]  # 提取第一个环境的物体点云

        # # 对点云进行采样
        # sampled_obj_points = sample_points(object_pointcloud.unsqueeze(0), sample_num=2048,
        #                                 sample_method='furthest_batch', device=self.device)

        # # 保存采样后的点云到 .npy 文件
        # np.save('object_pointcloud.npy', sampled_obj_points.squeeze(0).cpu().numpy())
...
# debug
    ...
    def reset(self, env_ids, goal_env_ids):
        ...
        if self.enable_data_collection and len(env_ids) > 0 and self.step_counter > 0:
            ...
            filtered_env_ids = env_ids[num_fingers_in_contact[env_ids] > 0]
            env_ids = filtered_env_ids
            for env_id in env_ids:
                # 标签是该 episode 的最终成功状态。
                ...
                if label == 1.0 and self.success_samples_collected < self.success_target:
                    ...
                    self.collected_point_clouds.append(current_points_fps[env_id])
