...
class ShadowHandRandomLoadVision(BaseTask,TacSLTaskImageAugmentation,TacSLSensors):
    def __init__(self, cfg, sim_params, physics_engine, device_type, device_id, headless,
        ...
        self.save_data_dir = "ECCV/rl_data"
        os.makedirs(self.save_data_dir, exist_ok=True)
        self.save_data_path = os.path.join(self.save_data_dir, "airplane-suc.npz")
        self.distance_trigger_threshold = 0.5 
        self.force_trigger_threshold = 0.01 
        self.success_target = 300
        self.fail_target = 0
        ...
        if self.enable_data_collection:
            ...
            self.temp_pc_buf = torch.zeros((self.num_envs, pc_num_points, 13), device=self.device)
    ...
    def compute_full_state(self, asymm_obs=False):
        ...
        num_ft_states = 13 * int(self.num_fingertips)
        num_ft_force_torques = 6 * int(self.num_fingertips)
    ...
    def _collect_pointclouds(self):
        ...
        if self.contact_calculation_enabled:
            ...
            if joint_angles.shape[1] == self.collision_model.n_dofs:
                ...
                has_contact_force = contact_force_magnitudes > self.force_trigger_threshold # (num_envs, 6)
                gaussian_sigma = 0.1 
                link_indices_map = {
                ...
                # Keep the legacy overwrite order: palm is written first, then fingers.
                # Therefore a finger ID overrides the palm ID on overlapping points.
    ...
    def reset(self, env_ids, goal_env_ids):
        if self.enable_data_collection and len(env_ids) > 0 and self.step_counter > 0:
            current_points_fps, _ = self._collect_pointclouds() 
            ...
            object_pose_batch = torch.eye(4, device=self.device).unsqueeze(0).repeat(self.num_envs,1,1)
            object_pose_batch[:,0:3,3] = object_pos
            object_pose_batch[:,0:3,0:3] = object_rot_matrix

            finger_forces_batch = self.finger_palm_contact_forces
            ...
            filtered_env_ids = env_ids[num_fingers_in_contact[env_ids] == self.cfg['env']['num_finger_contact']]
            env_ids = filtered_env_ids
            for env_id in env_ids:
                ...
                if label == 1.0 and self.success_samples_collected < self.success_target:
                    ...
                    print(f"Success sample collected at reset. Total success: {self.success_samples_collected}/{self.success_target}")

                    # Keep success samples in the same frame as fail samples and final inference.
                    self.collected_point_clouds.append(self.debug_points[env_id])
                    self.collected_labels.append(label)
                    self.collected_robot_states.append(rs_all[env_id])
                    self.collected_object_states.append(object_pose_batch[env_id]) 
                    self.collected_finger_forces.append(finger_forces_batch[env_id])
        ...
        dof_pos = self.shadow_hand_default_dof_pos 
