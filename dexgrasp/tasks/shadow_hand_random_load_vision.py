...
# collect data part
        ...
        self.save_data_path = "collected_grasp_data_50_50.npz" # 為新數據指定一個不同的文件名
        self.force_trigger_threshold = 1.0 # 觸發數據收集的接觸力大小閾值 (單位: 牛頓)

        # 新增：成功和失敗樣本的收集目標和計數器
        self.success_target = 50
        self.fail_target = 50
        ...
        if self.enable_data_collection:
            ...
            print(f"Trigger force threshold: {self.force_trigger_threshold} N")
            ...
            self.temp_pc_buf = torch.zeros((self.num_envs, pc_num_points, 6), device=self.device)
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        for i in range(self.num_envs):
            ...
            goal_handle = self.gym.create_actor(env_ptr, goal_asset, goal_start_pose, "goal_object", i + self.num_envs, 0, self.segmentation_id['goal'])
    ...
    def _load_cameras(self, env_ptr, env_id, camera_props, camera_eye_list, camera_lookat_list):
        ...
        for i in range(self.num_cameras):
            ...
            print(f"Environment {env_id}, Camera {i}: Eye = {camera_eye}, LookAt = {camera_lookat}")
    ...
    def compute_observations(self):
        ...
        self.finger_contact_forces = torch.stack([thumb_force, ff_force, mf_force, rf_force, lf_force], dim=1)
...
# 檢測是否有非零值
            # for k in ['palm_force_field', 'thumb_force_field','ff_force_field','mf_force_field','rf_force_field','lf_force_field']:
            #     image = tactile_force_field_dict[k]
            ...
            for k in ['palm_force_field', 'thumb_force_field','ff_force_field','mf_force_field','rf_force_field','lf_force_field']:
                ...
                # cv2.waitKey(1)
                image = image.reshape(self.num_envs, -1) / 255.0
                tactile_image_list.append(image)
            tactile_images = torch.cat(tactile_image_list, dim=-1)
            tactile_image_start = visual_feat_start + 64
            # 檢查image是否有非零值
            # tactile_image_start = point_cloud_start + self.num_pc_flatten + 2 * self.num_pc_downsample
            # self.obs_buf[:, tactile_image_start:tactile_image_start + 10800] = tactile_images
    def _collect_pointclouds(self):
        self.gym.render_all_camera_sensors(self.sim)
        self.gym.start_access_image_tensors(self.sim)

        output_dir = "camera_images_old"
        os.makedirs(output_dir, exist_ok=True)

        for env_idx in range(self.num_envs):
            # 獲取環境的 RGB 圖像 (RGBA格式)
            env_rgb_tensor = self.camera_rgb_tensor_list[env_idx][0]
            env_rgb_np = env_rgb_tensor.cpu().numpy()

            # 將圖像從 RGBA 轉換為 BGR 以便 OpenCV 保存
            env_bgr_np = cv2.cvtColor(env_rgb_np, cv2.COLOR_RGBA2BGR)

            # 定義保存路徑和文件名
            filename = os.path.join(output_dir, f"env_{env_idx}_camera.png")
            
            # 保存圖片
            cv2.imwrite(filename, env_bgr_np)
...
# debug
        self.debug_points = points_fps[:, :, 0:6].clone()
            ...
            for env_id in env_ids:
                if self.data_collected_buf[env_id]:
                    ...
                    if label == 1.0 and self.success_samples_collected < self.success_target:
                        ...
                        should_collect = True
                        print(f"Success sample collected. Total success: {self.success_samples_collected}/{self.success_target}")
    ...
    def post_physics_step(self):
        ...
        if self.enable_data_collection:
            ...
            if self.success_samples_collected < self.success_target or self.fail_samples_collected < self.fail_target:
                ...
                # 計算每個手指接觸力的範數(magnitude)
                # self.finger_contact_forces 的形狀是 (num_envs, 5_fingers, 3_xyz)
                contact_force_magnitudes = torch.norm(self.finger_contact_forces, p=2, dim=-1)
                
                # 找到每個 env 中最大的那個手指接觸力
                max_force_per_env, _ = torch.max(contact_force_magnitudes, dim=-1)
                # print("max", max_force_per_env)
                # 觸發條件 2: 最大接觸力超過閾值，且本 episode 尚未收集過數據
                triggered_envs_mask = (max_force_per_env > self.force_trigger_threshold) & ~self.data_collected_buf
                triggered_env_ids = torch.where(triggered_envs_mask)[0]

                if len(triggered_env_ids) > 0:
                    print(f"Contact force trigger activated for envs: {triggered_env_ids.cpu().numpy()}")
                    
                    # (後續的數據捕獲和暫存邏輯與之前相同)
                    point_clouds_to_cache = self.debug_points[triggered_env_ids]
                    tactile_force_field_dict = self.get_tactile_force_field_tensors_dict()
                    palm_tactile_data = tactile_force_field_dict['palm_force_field'][triggered_env_ids]
                    thumb_tactile_data = tactile_force_field_dict['thumb_force_field'][triggered_env_ids]
                    ff_tactile_data = tactile_force_field_dict['ff_force_field'][triggered_env_ids]
                    mf_tactile_data = tactile_force_field_dict['mf_force_field'][triggered_env_ids]
                    rf_tactile_data = tactile_force_field_dict['rf_force_field'][triggered_env_ids]
                    lf_tactile_data = tactile_force_field_dict['lf_force_field'][triggered_env_ids]
                    tactile_data = torch.stack([palm_tactile_data, thumb_tactile_data, ff_tactile_data, mf_tactile_data, rf_tactile_data, lf_tactile_data], dim=1)
                    processed_tactile_images = tactile_data.reshape(len(triggered_env_ids), -1)
                    self.temp_pc_buf[triggered_env_ids] = point_clouds_to_cache
                    self.temp_tactile_buf[triggered_env_ids] = processed_tactile_images
        ...
        # self.draw_point_cloud(self.debug_points)
        pc_to_save = self.debug_points[0].cpu().numpy()
        
        # 定義檔案名稱
        filename = "point_cloud.npy"
        
        # 儲存為 .npy 檔案
        np.save(filename, pc_to_save)

    def draw_point_cloud(self, points):
        """
        Visualizes the point cloud in the Isaac Gym viewer.
        Args:
            points (torch.Tensor): A tensor of shape (num_envs, num_points, 6)
                                   containing xyz and rgb data.
        """
        if not self.viewer:
            return
        ...
        # 獲取點的 XYZ 和 RGB 數據
        points_xyz = points[env_idx, :, :3].cpu().numpy()
        points_rgb = points[env_idx, :, 3:6].cpu().numpy()

        # 將點轉換為 gym.Vec3 格式
        verts = []
        colors = []
        for i in range(points_xyz.shape[0]):
            point_xyz = points_xyz[i]
            point_rgb = points_rgb[i]
            
            # 為了畫出一個點，我們創建一個長度極短的線
            p1 = gymapi.Vec3(point_xyz[0], point_xyz[1], point_xyz[2])
            p2 = gymapi.Vec3(point_xyz[0], point_xyz[1], point_xyz[2] + 0.001) # 稍微增加一點高度
            
            verts.append(p1)
            verts.append(p2)
            colors.append(gymapi.Vec3(point_rgb[0], point_rgb[1], point_rgb[2]))

        # 一次性繪製所有線條
        if len(verts) > 0:
            self.gym.add_lines(self.viewer, env_ptr, len(colors), verts, colors)
...
# 在 ShadowHandRandomLoadVision 類中任意位置添加這兩個方法

    def _process_tactile_image(self, tactile_tensor):
        """
        處理手掌觸覺圖像，將其轉換為分類器所需的格式 (B, C, H, W)。
        palm_tactile_tensor 的原始形狀應為 (B, H, W, C)。
        """
        if self.TF is None:
            return None
        
        # 1. 從 GPU 轉到 CPU 並轉換為 (B, C, H, W)
        # 原始力場圖可能是偽彩圖，所以有3個通道
        img_tensor = tactile_tensor.permute(0, 3, 1, 2).cpu() # (B, C, H, W)

        # 2. 縮放到 (64, 64)
        resized_images = self.TF.resize(img_tensor, [64, 64])

        # 3. 歸一化到 [0, 1] (如果需要的話，原始get_tactile_force_field_tensors_dict已經處理了)
        # resized_images = resized_images / 255.0

        return resized_images.to(self.device)
    ...
    def _save_collected_data(self):
        ...
        tactile_images_np = torch.stack(self.collected_tactile_images).cpu().numpy()
        ...
        if os.path.exists(self.save_data_path):
            with np.load(self.save_data_path) as old_data:
                ...
                tactile_images_np = np.concatenate((old_data['tactile_images'], tactile_images_np), axis=0)
                labels_np = np.concatenate((old_data['labels'], labels_np), axis=0)

        # 保存數據
        np.savez(self.save_data_path,
                 point_clouds=point_clouds_np,
                 tactile_images=tactile_images_np,
                 labels=labels_np)
        ...
        self.collected_tactile_images.clear()
