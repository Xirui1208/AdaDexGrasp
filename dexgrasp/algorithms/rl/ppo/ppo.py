...
class PPO:
    ...
    def __init__(self,
                 ...
                 asymmetric=False,
                 is_vision = False,
                 is_tactile_image = True
                 ):
        self.is_vision = is_vision
        self.is_tactile_image = is_tactile_image
        ...
        self.actor_critic = actor_critic_class(self.observation_space.shape, self.state_space.shape, self.action_space.shape,
                                               init_noise_std, model_cfg, asymmetric=asymmetric, use_pc = self.is_vision, is_tactile_image = self.is_tactile_image)
        ...
        if self.is_testing:
            ...
            for it in range(self.current_learning_iteration, num_learning_iterations):
                ...
                for _ in range(self.num_transitions_per_env):
                    ...
                    # 1. 检查输入给网络的观测值(observation)和状态(states)
                    if torch.isnan(current_obs).any() or torch.isinf(current_obs).any():
                        print("!!! FATAL: Input 'current_obs' to actor_critic contains NaN/inf!")
                        # 直接抛出异常，让程序停在这里
                        raise ValueError("NaN/inf detected in current_obs")
                    if current_states is not None and (torch.isnan(current_states).any() or torch.isinf(current_states).any()):
                        print("!!! FATAL: Input 'current_states' to actor_critic contains NaN/inf!")
                        raise ValueError("NaN/inf detected in current_states")


                    # Compute the action
                    actions, actions_log_prob, values, mu, sigma = self.actor_critic.act(current_obs, current_states)
                    if torch.isnan(mu).any() or torch.isinf(mu).any():
                        print("!!! FATAL: Network output 'mu' (actions_mean) contains NaN/inf!")
                        raise ValueError("NaN/inf detected in mu")
                    if torch.isnan(sigma).any() or torch.isinf(sigma).any():
                        print("!!! FATAL: Network output 'sigma' (standard deviation) contains NaN/inf!")
                        raise ValueError("NaN/inf detected in sigma")
