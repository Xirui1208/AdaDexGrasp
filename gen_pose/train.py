...
from model import NetworkRegression  
from dataset.dataset import RegressionGraspDataset as Dataset
...
def train(task, config, data_dir="data", base_dir="logs"):
    ...
    print("Creating network (for Regression Task) ...... ")
    network = NetworkRegression(
    ...
    # start training
    start_epoch = 1
    print("Start training for Regression Task ...... ")
    ...
    for epoch in tqdm(
        ...
        content = {
            "MSE_loss": train_ep_loss / train_cnt,
...
def forward_step(batch, network: NetworkRegression, device, epoch=0, is_val=False):

    pc_with_mask, target_robot_state = batch
    pcs = pc_with_mask.to(device)     # (B, N, 4)
    targets = target_robot_state.to(device)  
    losses, predictions = network.get_loss(pcs, targets)
...
if __name__ == "__main__":
    ...
    args.add_argument("--task", type=str, default="final", help="task name for regression")
    args.add_argument(
    ...
    config = {
        ...
        "point_cloud_dim": 4,  # (X, Y, Z, ContactMask)
        "robot_state_dim": 29, 
        "feat_dim": 128,
        "lr": 0.001,
        "weight_decay": 1e-5,
    ...
    data_dir = "gen_pose/data"
    base_dir = "gen_pose/logs"
